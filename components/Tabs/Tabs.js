...
class Tabs {
  constructor(element) {
    ...
    this.links = this.links.map((index, link) => {
      return new TabsLink($(link), this);  //we go through tabs-link 1-4 and pass each one to TabsLink
    });

    // Select the first Link and set it to the activeLink
    this.activeLink = $(`.tabs-links[data-tab= "1"]`);
    this.init();
  }
  ...
  getTab(data) {
    ...
    return this.element.fin(`.tabs-item[data-tab="${data}"]`);
